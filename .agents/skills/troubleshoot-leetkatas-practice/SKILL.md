...
4. Read `references/subsystems.md` when the failing boundary is unclear.

Do not expose submitted source, metadata, credentials, environment secrets, or database contents in the response. The normal log intentionally omits source and metadata bodies.
...
4. Classify the failing boundary: launcher/environment, selection/SQLite/FSRS, compiler, reviewer protocol/Codex, Neovim response validation/UI, or rating persistence.
5. State the evidence and root cause before changing code. Implement a fix only when the user requested one or the active request clearly includes repair.
